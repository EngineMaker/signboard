...
> ステータス: **フェーズ3 MVP 完成 (2026-09-20)** — https://signboard.emaker.dev 稼働中
> 最終更新: 2026-09-20
...
### Step 7: Discord Bot ✅ 完了・実コマンド確認済み (2026-09-20)
...
### Step 8: API キー ★要レビュー ✅ 完了・レビュー承認済み (2026-09-20)

- [x] 管理画面からキー発行・失効（**保存は SHA-256 ハッシュ、平文は発行時のみ表示**）
- [x] `Authorization: Bearer` でお知らせ CRUD（`/api/v1/notices`）
- [x] API 経由も監査ログに `source: api` で記録
- [x] 最終使用日時を記録（使われていないキーを見分けられる）
- [x] 管理画面に curl の使用例を表示

**AC**: `curl -H "Authorization: Bearer $KEY" ...` で投稿でき、無効キーは401 / DB内に平文キーが存在しないことをテストで検証

### Step 9: インフラのコード化とデプロイ ★要レビュー ✅ 完了 (2026-09-20)
...
**MVP 完成 (2026-09-20)**: Step 1〜9 すべて完了。本番稼働中。
本番URLでの Discord ログインも確認済み。
...
- **実サーバーでの確認**:
  - 無効なキー → 401
  - 有効なキーで投稿 → 201、掲示板にも反映
  - **DB の全テーブルを走査し、平文がどこにも存在しないことを確認**
  - ハッシュのみ保存されていることを確認
- ブラウザで発行・一覧・使用例表示を確認。JSエラーなし
...
**実コマンド確認**: Discord から `/signboard post` を実行し、iPad の掲示板に即座に反映されることを確認済み。
